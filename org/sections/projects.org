...
*** AMD Pensando DSC Live Migration
:PROPERTIES:
:CV_ENV: cvproject
:PROJECT: AMD Pensando SmartNIC Platform
:DATE: Jan. 2022 -- Present
:LANGUAGES: C, C++, P4, Python
:TOOLS: Git, GDB, Qemu, Xen Hypervisor, DPDK, Linux Kernel
:TECH: VFIO, VDPA, IOMMU, NVMe-oF, SmartNIC, DPU, P4+, virtio, Packet Processing Pipelines
:END:

- Synopsis :: Delivered fully supported live-migration feature for the AMD Pensando DSC SmartNIC across all host-visible device modes — network, virtio, and NVMe. Work spanned Linux kernel control-path frameworks, P4+-based dataplane with hardware dirty-page tracking, and scalability validation on Elba ASICs to achieve zero-downtime VM migration in cloud environments.
- Details :: Engineered vendor control-path support across the *Linux VFIO and vDPA frameworks*, ensuring correct migration sequencing for all device presentation modes. Implemented the migration datapath entirely in *P4+ language* with *hardware-level dirty-page tracking* to minimize pre-copy transfer volume and downtime. For the vDPA path, integrated *system IOMMU dirty-tracking* as the page-change source in place of device hardware, fully closing the feature gap. Validated both VFIO and vDPA combinations end-to-end using *QEMU-orchestrated migration across multiple physical hosts*. Subsequently investigated *scalability bottlenecks* on AMD/Pensando Elba ASICs by tracing dirty-page reporting through the *SEQ/ACK bitmap protocol* in the dataplane, firmware, and host migration drivers. Built an *end-to-end validation framework* for NVMe + virtio-pci VM topologies with automated QEMU launch, VF bind checks, hugepage preparation, and *QMP-driven pre-copy / stop-and-copy orchestration*.
...
- Synopsis :: The DPU presents itself as a PCIe device, all access from the host ends up in indirect access and handled in software stack. Optimizting the PMT/PRT entries to make most of the hardware access end up as direct memory access.
...
:PROPERTIES:
:CV_ENV: cvproject
:PROJECT: Model0, Pre-Silicon validation
